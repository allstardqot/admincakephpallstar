...
class UsersTable extends Table {
	...
	public function initialize(array $config) {
		...
		$this->hasOne('ReferalCodeDetails');
		$this->hasOne('PenAadharCard');
		$this->hasOne('BankDetails');
		$this->hasOne('AadharCard');
		$this->hasOne('WithdrawRequests');
		
		$this->hasMany('ReferedByReferalDetails', [
			'className'	=>	'App\Model\Table\ReferalCodeDetailsTable',
			...
			->add('name', 'Search.Callback', [
                'callback' => function ($query, $args, $filter) {
					...
					foreach($args as $name) {
						$filterArr[]['OR']	=	['first_name LIKE'=>'%'.$name.'%','last_name LIKE'=>'%'.$name.'%','Users.team_name LIKE'=>'%'.$name.'%','Users.refer_id LIKE'=>'%'.$name.'%'];
					}
	...
	public function validationUserMobile(Validator $validator) {

		$validator
				->add('phone', 'unique', ['rule' => 'validateUnique', 'provider' => 'table', 'message' => __('Mobile number already exists.')]);

		$validator
				->notEmpty('phone', __('Contact is required'))
				->add('phone', [
					'phone' => [
						'rule' => ['numeric'],
						'message' => __('Contact number should be numeric.')
					]
				])
				->add('phone', [
					/*  'numeric' => [
					...
					'minLength' => [
						'rule' => ['minLength', 10],
						'last' => true,
						'message' => __('Contact number must be at least 10 digit')
					],
					'maxLength' => [
						'rule' => ['maxLength', 15],
	...
	public function validationOtp(Validator $validator) {
		$validator
			->notEmpty('otp', 'OTP cannot be empty');
		return $validator;
    }
	...
	public function validationPassword(Validator $validator) {
		...
		$validator
			->add('password', 'custom', [
				...
				if ($user) 
				{
					if((new WeakPasswordHasher)->check($value, $user->password)) {
						return true;
					}
				}
				return false;
	...
	public function validationAddEditUser(Validator $validator) {
		$validator
				->notEmpty('first_name', __('First name is required'));
		$validator
				->notEmpty('last_name', __('Last name is required'));
		$validator
				->notEmpty('country_code', __('Country code is required'));
		$validator
				->requirePresence('email')
				->notEmpty('email', __('Email is required'))
				->add('email', 'validFormat', ['rule' => 'email', 'message' => __('E-mail must be valid')])
	...
	public function validationSignup(Validator $validator) {
		...
		$validator
				...
				->add('phone', 'unique', ['rule' => 'validateUnique', 'provider' => 'table', 'message' => __('Phone already exist')]);
		$validator
				->notEmpty('password', __('Password is required'))
				->add('password', [
					'minLength' => [
	...
	public function validationLoginPassword(Validator $validator) {
        $validator
			->add('password', 'custom', [
				'rule' => function($value, $context) {
					$user = $this->find()->where(['phone'=>$context['data']['phone']])->first();
					if ($user) {
						if((new WeakPasswordHasher)->check($value, $user->password)) {
							return true;
						}
