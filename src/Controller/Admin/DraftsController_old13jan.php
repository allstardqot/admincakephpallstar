...
class DraftsController extends AppController {
	...
	public function addnew() {
		...
		if ($this->request->is(['patch', 'post', 'put'])) {
			...
			if(!empty($dataGet)){
				...
				//pr($date_teamid);pr($dataGet);die;
				if(!empty($dataGet['week']) && !empty($date_teamid)){
					$this->loadModel('ApiPlayers');
					//$condition=array('ApiPlayers.Week'=> $dataGet['week']);
					//$date_teamid=json_encode($date_teamid);
					$condition=['ApiPlayers.Week'=> $dataGet['week'],'ApiPlayers.PlayerID IN'=> $date_teamid];
					//pr($condition);
	...
	public function editdraft($eid=null) {
		...
		$this->set(compact('seriesData','weeks'));
	}
