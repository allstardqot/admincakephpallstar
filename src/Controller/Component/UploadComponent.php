...
class UploadComponent extends Component {
    ...
    public function upload($file, $destination, $name = NULL, $rules = NULL, $allowed = NULL) {
        ...
        if (!is_null($allowed)) {
            ...
            //echo "Else";
            $this->_allowed = array('jpg', 'jpeg', 'gif', 'png', 'JPG', 'JPEG', 'GIF', 'PNG', '.flv', '.swf', 'mp3','xml','pdf','docx','doc');
        }
    ...
    public function uniquename($file) {
        ...
        $file = strtolower($parts['filename']);
        $ext = $parts['extension'];
        if ($ext) {
            $ext = '.' . $ext;
    ...
    function resizeSmart($source_image, $destination, $tn_w, $tn_h, $quality = 100, $wmsource = false)
    {
        // by pankaj Gupta
    ...
    public function get_new_name($filename, $length = 20, $prefix = "") {
        $ext = pathinfo($filename, PATHINFO_EXTENSION);
        if ($prefix) {
            $length -= strlen($prefix) + strlen($ext);
            if ($length <= 0) {
                $length = 20;
            }
        }
        $new_name = substr(uniqid($prefix), 0, $length) . '.' . $ext;
        return $new_name;
    }
